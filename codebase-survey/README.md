...
3. **`/codebase-architecture-assessment`** — *user-only*. Cross-cutting pass after modules are documented: domain-boundary leaks, coupling hotspots, deviations between stated and actual architecture. Writes findings to `docs/codebase/assessment.md` (tagged `kind: rule` / `kind: observation`) **and** synthesises the three sibling docs in the same directory — `architecture.md` (narrative system description), `tech-stack.md` (languages/frameworks roll-up), and `operations.md` (CI/CD, deploys, secrets, observability — sourced from a repo-level `ops-detective` run plus per-module Operations sections). Files lacking the `generated_by` front-matter marker are treated as human-authored stated content and not overwritten.
