...
For each resulting task (the single refined task, or each split child), set `status: todo`, `context: <slug>`, and wire `depends_on` to the ids the interview identified. The task body must have exactly these sections, in this order and at these heading levels:
