...
- **Unintegrated external systems.** An external context on the map (or a system in the domain model's `## External systems` list that never made it onto the map — itself a `/context-mapping` gap to flag) with no live or `done` task in its owned facing context building the integration; or an external boundary whose work is underway but whose behavior no dossier covers (route that to `/dossier`, not a task). External boundaries are classic root work: a policy reacting to an `(external)` event can't land before the event arrives.
