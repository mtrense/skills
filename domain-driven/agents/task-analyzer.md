...
- **Size** — one implementation pass, or several? If several, propose a concrete split into named sub-tasks with a one-line outcome each and any ordering among them. Name each piece by its **outcome/slug only** (e.g. *parse-config*, *validate-schema*) — **never** by suffixing the task's id (`0001a`/`0001b`, `0001-1`): child ids do not exist yet; the orchestrator mints fresh sequential ids via `tasks.sh next-id` later, and suffix labels in your report leak into filenames that break the loader.
- **Complexity** — how hard the pass is to get *right*, as `low`, `medium`, or `high`, with a one-line justification. This is **reasoning difficulty, not volume** — orthogonal to Size: a broad-but-mechanical change (a rename across many files, boilerplate following an established pattern) is `low` even when it touches a lot; a small change is `high` when it demands novel design, sits on an ambiguous or load-bearing seam, involves subtle invariants/concurrency/edge-case reasoning, or risks breaking things far from the diff. Calibrate roughly: `low` — the codebase already shows the pattern and the task follows it; `medium` — some design judgment, but within established seams; `high` — new design territory, tricky correctness reasoning, or high blast radius. When you propose a split, estimate each proposed child separately. The orchestrator persists the settled value in frontmatter so a later `/task-cycle` can route on it — keep it abstract (never a model or agent recommendation).
...
- **Dossiers** — the dossiers bearing on this task, by slug, with the specific claims that constrain it (quote the claim with its confidence). Flag two things: a task leaning on a **`low`-confidence claim** in its acceptance criteria (the human should know the fact is soft), and an **under-researched task** — one whose outcome hinges on external facts (a regulation's requirements, an undocumented API's behavior, a market rule) that no dossier covers; recommend a `/dossier` run on the subject first. This is a flag, not a gate. Also surface any **open unknown** in a bearing dossier that this task would trip over. Omit the section when nothing applies.
