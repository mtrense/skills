...
6. **`/task-refine`** — turns a `draft` into a ready `todo`: assesses completeness, **domain-compliance**, size, and **complexity** — an abstract `low | medium | high` reasoning-difficulty estimate, orthogonal to size, persisted in frontmatter as the routing hint `/task-cycle` dispatches on (delegated to the `task-analyzer` subagent); interviews the human; **splits** oversized tasks; wires **dependencies**; surfaces decisions and attaches ADRs.
7. **`/task-cycle`** — drives ready `todo` tasks to `done` via `task-worker` subagents (strict TDD → verify → commit), each dispatched on the model tier its task's `complexity` maps to (default `low=sonnet, medium=sonnet, high=opus`, project-overridable via the `.workflow-overrides/model-map` file, resolved deterministically at skill invocation via `!`-command preprocessing), with one escalation retry at the next tier up when a worker fails. `all@1` works in-place sequentially; `@N` implements in parallel git worktrees and merges back sequentially via the `integrator` subagent (bounce-on-conflict). When a landed task's *Deviations from plan* record is non-trivial, sets `deviated: true` in its frontmatter — producing the drift worklist the revision runs consume. Has an **experimental workflow-backed twin**, `/task-cycle-run` (`workflows/task-cycle.js`), that runs the same burn-down as a background Workflow so the main session stays free for `/task-append`/`/task-refine` — see *Concurrency & status ownership* below.
...
architecture/                 # architecture home (shared convention with common/adr; default dir, override via the `.workflow-overrides/architecture-path` file)
...
complexity: medium            # low | medium | high — reasoning difficulty of the pass, not volume (empty until refined; abstract, never a model name — the routing hint /task-cycle maps to a model tier at dispatch)
...
The backlog can grow large. **No skill or subagent ever reads the task corpus (or a filtered slice of it) by scanning files.** Every question about the backlog is answered by the deterministic **`tasks.sh`** helper (bundled in the `task-status` skill directory), which parses only YAML frontmatter (`yj -yj` → `jq`) and returns ids/paths/counts. Task **frontmatter is the query index; prose bodies are read only by the one worker implementing that one task** — with two sanctioned exceptions, both bounded, status-gated worklists rather than scans: a `/domain-model`, `/context-mapping`, or `/architecture-foundation` revision reads the `## Closing` sections of exactly the ids `tasks.sh deviated` lists, and `/showcase` reads the `## Closing` sections of exactly the ids `tasks.sh by-status done` lists (pre-extracted at invocation, `### Manual testing` being the raw material of its walk-through).
...
- **`/task-cycle-run` = the background variant, safe next to a live session.** Same `task-worker` + `integrator` contract and `[<limit>|all][@<workers>]` argument, but designed to run while the human keeps capturing and refining in the main session: it **always** uses worktree mode (even `@1`), so a worker's commit can never sweep the human's uncommitted main-checkout drafts into a code commit; it drops the blanket dirty-tree refusal — instead every main-checkout write goes through a serial **scribe** agent committing with an explicit pathspec (`git commit … -- <file>`, only ever the one task file), and a ready task whose own file has uncommitted edits is skipped for the pass. The single-writer invariant holds one indirection deeper: a Workflow script has no filesystem/bash, so one scribe agent at a time — invoked only between batches — is the sole status/task-file writer, with gate agents standing in for the in-session `tasks.sh`/git reads. A task that fails post-escalation or bounces twice in one run is left in `todo` and excluded from that run's later passes, so `all` mode cannot loop forever; because every pass re-derives the ready-set from disk, tasks refined to `todo` *while it runs* are picked up automatically.
...
- **`task-analyzer`** — refine assessment (completeness, domain-compliance, size, complexity, deps, decisions/ADRs, bearing exemplars) for one task.
