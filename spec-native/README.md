...
- **NFRs** — measurable cross-cutting properties (latency, error rate, throughput, availability, accessibility level, extensibility surface, error-recovery time, operability/MTTR, observability coverage, security posture).
- **Constraints** — non-measurable external requirements (regulatory, contractual, compatibility).
...
**What `Evidence` contains.** Evidence describes *what an outside observer can detect when the system response occurs* — the externally-visible signals that distinguish "behavior happened correctly" from "behavior didn't happen". Concretely: HTTP status + response shape, emitted events/log lines with key fields, state transitions visible via the public API, user-visible UI changes, or measurable outcomes (counter incremented, mail sent). Evidence is **prose, not a test reference** — tests verify evidence; they are not it. The link between behavior and verifying test is carried by `// SPEC:` annotations on tests (see *Code-side annotations*), not by embedding test IDs in the spec.
...
- **`nfr/api.md`** — grouped measurable properties, each as a heading: `### p95-latency` (full ID `NFR:api#p95-latency`). Each has Statement / Measurement / Threshold / Rationale.
- **`constraints/gdpr.md`** — same shape as NFR but for non-measurable external constraints. Each has Statement / Source / Verification.
...
- **`glossary.md`** — flat file, one `### <slug>` heading per term, each carrying a short definition. IDs are `G:#<slug>` (anchor-only form; the glossary is a singleton file). Spec prose references terms via plain markdown links (`[idempotency](../glossary.md#idempotency)`) rather than a new marker — the `G:` prefix exists so frontmatter `references` and the reverse-trace tool treat glossary entries uniformly with NFRs, constraints, and DM entities. Authoring discipline: italicize a term-of-art on first use per file and link it; subsequent mentions in the same file are bare. `/spec-glossary-sync` enforces three things: (a) every italicized first-use resolves to a `G:` entry (or proposes adding one), (b) every `G:` entry is referenced from ≥1 spec file (orphans flagged), (c) terms used heavily without italic-link on first use get an AUDIT under `ambiguity`.
