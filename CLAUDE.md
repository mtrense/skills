...
- `material-extractor` — pulls atomic knowledge units from `reference/` files (each with candidate prerequisites, a "why it matters" hook, and a source line) and reports which grounding kind applies (research vs document). For `/author-ingest` (one call for a small corpus, fanned out per file/subtree for a large one) and `/author-ingest-update` (which runs it over only the changed source files).
