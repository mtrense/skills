...
- **Mode divergence** — per accent role (`primary`, status colors): how dark diverges from light (lightness/saturation shift) and why. Reusing a hex unchanged across modes must be called out as a deliberate choice per the conventions — never left implicit.
- **Large-fill note** — one line per mode on how `primary` reads as a large filled area against `bg` (a button bar, a selected band): sits in the palette, or risks reading pasted-in. Contrast math doesn't catch a jarring accent; you flag it here so the interview's prototype round can verify by eye.
