...
Accessibility and i18n are baked in, not bolted on: WCAG 2.2 AA is the floor (every `on-<x>`/`<x>` token pair ≥ 4.5:1, semantic elements first, `focus-visible` everywhere), and every component block ships an RTL sample (`dir="rtl"`), a long-string sample (`data-ds-sample="long"`), locale-format slots (`data-ds-locale`), and logical direction classes only (`ms-*`/`text-start`/…, with `data-ds-physical` as the explicit opt-out). Within a block, every discrete sample is wrapped in a `data-ds-story="<slug>"` element (`variant-<v>`, `state-<s>`, `rtl`, `long-string`, free-form extras) — the machine-readable story split that lets the gate, revisions, and downstream consumers (e.g. a storybook-style viewer) address one sample without parsing prose. The canonical contract is the bundled conventions doc: `skills/design-audit/references/conventions.md`.
...
**`design-check.sh`** (in `skills/design-audit/`) — the deterministic quality gate: `bash <skills-root>/design-audit/design-check.sh <ds-dir> [<theme-dir> …]`. Per theme it verifies the required `--ds-*` color roles, computes actual WCAG contrast (≥ 4.5:1) for every `on-<x>`/`<x>` hex pair, and checks `lang`/`dir` on `<html>`, the CDN + stylesheet links, marker integrity against the catalog, per-block samples (RTL, long-string, ARIA, `focus-visible`, no physical direction classes), and the story markers — `data-ds-story` slugs unique within a block (FAIL on duplicates), one `variant-<slug>` story per variant in the spec's machine-readable variants list (WARN when absent), story sets identical across theme dirs (FAIL on mismatch; blocks with no stories at all WARN as pre-story builds). Output is one `OK|WARN|FAIL|MISS` line per finding; exit 1 iff any FAIL. The `visual-critic` subagent is its complement — it judges what math can't (hierarchy, rhythm, dark-mode legibility, personality fidelity).
