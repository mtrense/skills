...
| 7 | `/task-cycle` | Drive ready `todo` tasks to `done` via `task-worker` (TDD → verify → commit). `[<limit>\|all][@<workers>]` (default `all@1`); `@N` uses parallel git worktrees + serial `integrator` merge-back |
