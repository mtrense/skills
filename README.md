...
| `/adr` | Manually record one or more ADRs from the current conversation under the project's decisions directory (`decisions/` by default, overridable via `decision-path:` in `CLAUDE.md`) — the human override for when a decision worth preserving was made in-session but no skill recorded it |
