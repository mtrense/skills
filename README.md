...
| `/version-bump` | Bump the project's version (detects `package.json`, `Cargo.toml`, `pyproject.toml`, and similar) from `major`/`minor`/`patch`, an explicit `x.y.z`, or `current` (no bump — tag the version the manifest already declares, for cutting a scaffolded project's first release), then cut an annotated git tag — gathers a changelog from the commits since the last tag and writes it to CHANGELOG.md, the tag message, and the chat; defers the commit to `/commit` |
