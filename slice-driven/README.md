...
| `/decide` | Records decisions with a grounding tier; gatekeeper for reopening settled ones. Argless, it takes on the next `provisional` record and works it through a Socratic dialogue to one named outcome — sharpen, ground (only against a real evidence link), supersede, probe, or retire. | `decisions/NNNN-slug.md` + `DECISIONS.md` |
