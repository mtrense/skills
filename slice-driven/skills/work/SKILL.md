...
- **Tasks** (features only) — ordered, each independently testable and sized for one TDD session, **walking-skeleton first**: task 1 always produces something runnable end-to-end, however thin. Co-edit with the user; they reorder and veto. Chores get a task list only if genuinely multi-step.
